...
class VideosRepository {
    ...
    getVideos(request, response) {
        ...
        mysql_1.pool.getConnection((err, connection) => {
            connection.query('SELECT * FROM videos WHERE user_id = ?', [user_id], (err, result, fields) => {
                connection.release();
                if (err) {
    ...
    searchVideos(request, response) {
        ...
        mysql_1.pool.getConnection((err, connection) => {
            connection.query('SELECT * FROM videos WHERE title LIKE ?', [`%${search}%`], (err, result, fields) => {
                connection.release();
                if (err) {
                    return response.status(400).json({ err: 'Erro ao buscar vídeos!' });
                }
